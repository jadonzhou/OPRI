...
Dx = pd.read_csv("/Volumes/T7/OPRI UK/Objective 9/cprd_out_inpatient_data20142015.csv")
...
for p in range(database.shape[0]):
    ...
    comorbidities=comorbidities.sort_values(by = 'admidate',ascending=False)
    baselineDate=database.iloc[p,1]
    #comorbidities = comorbidities[(comorbidities['admidate']<=pd.to_datetime(baselineDate))]
    for i in range(comorbidities.shape[0]):
        ...
        for j in range(comCategory.shape[1]):
            if code in comCategory.iloc[:,j].dropna().tolist() or str(code) in comCategory.iloc[:,j].dropna().tolist():
                ...
                result_date.iloc[p,j]=comorbidities.iloc[i,4]
